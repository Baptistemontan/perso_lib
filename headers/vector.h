 ...
 * exemple: if you give arr + x to vec_size, it will segfault if x != 0.
...
#define VEC_DEF_PUSHBACK(type, suffix) \
    inline type vec_pushBack_##suffix(type** _arrPtr, type _value) { \
        _vec_priv_pushBack((void**)_arrPtr, &_value); \
        return _value; \
    }
...
#define VEC_DEF_PUSHFRONT(type, suffix) \
    inline type vec_pushFront_##suffix(type** _arrPtr, type _value) { \
        _vec_priv_pushFront((void**)_arrPtr, &_value); \
        return _value; \
    }
...
#define VEC_DEF_POPBACK(type, suffix) \
    inline type vec_popBack_##suffix(type** _arrPtr) { \
        type _buff; \
        _vec_priv_popBack((void**)_arrPtr, &_buff); \
        return _buff; \
    }
...
#define VEC_DEF_POPFRONT(type, suffix) \
    inline type vec_popFront_##suffix(type** _arrPtr) { \
        type _buff; \
        _vec_priv_popFront((void**)_arrPtr, &_buff); \
        return _buff; \
    }
...
#define VEC_DEF_SLICE(type, suffix) \
    inline type* vec_slice_##suffix(type* _arr, size_t _start, size_t _end) { \
        return (type*)_vec_priv_slice(_arr, _start, _end); \
    }
...
#define VEC_DEF_INSERT(type, suffix) \
    inline type vec_insert_##suffix(type** _arrPtr, size_t _index, type _value) { \
        _vec_priv_insert((void**)_arrPtr, _index, &_value); \
        return _value; \
    }
...
#define VEC_DEF_REMOVE(type, suffix) \
    inline type vec_remove_##suffix(type** _arrPtr, size_t _index) { \
        type _buff; \
        _vec_priv_remove((void**)_arrPtr, _index, &_buff); \
        return _buff; \
    }
...
#define VEC_DEF_BSEARCH(type, suffix) \
    inline type* vec_bsearch_##suffix(type* _arr, type _value, int (*_compar_fn) (const void *, const void *)) { \
        size_t _size = vec_size(_arr); \
...
#define VEC_DEF_CLEAR(type, suffix) \
    inline void vec_clear_##suffix(type** _arrPtr) { \
        _vec_priv_clear((void**)_arrPtr); \
    }
...
#define VEC_DEF_MAP(fromType, toType, suffix) \
    toType* vec_map_##suffix(fromType* arr, toType (*map_fn)(fromType, size_t)) { \
        size_t arrSize = vec_size(arr); \
...
// vec_foreach(arr, int, i, val, 
//     printf("%d\n", val);
// )
// val is the value of the current element
// i is the index of the current element
#define vec_foreach(arr, type, iter, val, loop) \
    { \
        type val; \
        for(size_t iter = 0; iter < vec_size(arr); iter++) {\
            val = arr[iter]; \
            loop \
        } \
    }
...
#define vec_foreach_reverse(arr, type, iter, val, loop) \
    { \
        type val; \
        for(size_t iter = vec_size(arr) - 1; iter >= 0; iter--) {\
            val = arr[iter]; \
            loop \
        } \
    }
...
void vec_swap(void* arrPtr, size_t index1, size_t index2);
...
void _vec_priv_remove(void** arrPtr, size_t index, void* buff);
void _vec_priv_clear(void** arrPtr);
void _vec_debug_print(void* arr, FILE* stream); // write informations about the array to the given stream, for debug purpose
